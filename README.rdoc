...
2. in paradigms#dump, allow filtering
   * get only paradigms with status=ready
   * what should be the default? dumping only ready paradigms?
...
5. Now dumping paradigms does not require authentication. Implement one for it? consider Basic HTTP Authentication

...
1. INPROGRESS: a new model Inflector serves to convert given word with the given tag to the given tags
	...
	* DISMISSED: run syn_ldb as a TCP server? -- scripts are in bin/syn_ldb_server
	* NEXT: any encoding issues: web app expects UTF8 meanwhile prosao1 returns CP1252? test with FSE
	* NEXT: fix letter case of words returned from prosao1 syn_ldb if necessary
...
2. INPROGRESS: implement converting words in paradigm#edit
	...
	* DONE: do not submit the form if there are no empty slots

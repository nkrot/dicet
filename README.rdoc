...
6. assign task statuses, more particularly, READY, and hide READY tasks by default. display user tasks in three categories (and with count stats):
	* done tasks (not displayed by default)
	* tasks in progress (having words done by this user). if a task has a word that is also in another user's task, this task should not be classified as 'in progress'
	* not started tasks.
	* allow sorting tasks by last modification date
...
1. DONE: a new model Inflector serves to convert given word with the given tag to the given tags
	* DONE: Inflector#convert
	* DONE: how to configure the external script to be used? -- syn_ldb available in the environment is used in Inflector (hardcoded)
	* DONE: run syn_ldb as a TCP server? -- scripts are in bin/syn_ldb_server
	* DONE: Let Inflector discover syn_ldb from the environment or as tcp server
	* DONE: any encoding issues: web app expects UTF8 meanwhile prosao1 returns CP1252? test with FSE

2. INPROGRESS: implement converting words in paradigm#edit
	* DONE: add button (<) for inserting current word into the desired word box
	* DONE: add button (convert) for triggering conversion procedure
	* DONE: use ajax to serve conversion procedure.
	* BUG: try CONVERT on an existing paradigm and a newly opened one
	* DONE: double clicking on (<) button should trigger conversion procedure.
	* TODO: double click does not prevent two single clicks from happening. it does not harm however
	* DONE: do not submit the form to CONVERT if there are no empty slots

3. INPROGRESS: Implement ParadigmForm to hold both the old and the new state
   * DONE: create it from params
   * DONE: create it from Paradigm object
   * write tests spec/model/paradigm_form_slot_spec.rb
   * write tests spec/model/paradigm_form_spec.rb
   * DONE: switch all views to using ParadigmForm instead of Paradigm
   * DONE: implement save_paradigm and update_paradigm using ParadigmForm#save
   * remove original code for save_paradigm and update_paradigm
   * write cool tests for ParadigmForm#save
   * DONE: use ParadigmForm in CONVERT

4. DONE. Ensure that ParadigmForm contains all tags that the paradigm of the given type must have plus user-added tags. And in the correct order!

5. BUG: create a paradigm with NN and NNS. create an empty slot for NNS. press CONVERT. the 2nd slot for NNS is simply deleted. looks like ParadigmForm#sanitize overworks

6. refactor paradigm_for_form view to replace @paradigm with @paradigm_form or get rid of explicitly passing @paradigm

7. INPROGRESS: Ensure that all changed word forms (including those that were before CONVERT was pressed) are highlighted as changed. Ensure that all other events still work.

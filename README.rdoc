...
8. In paradigms#new, allow jumping to editing the next word from the task. Implement is as a side pane listing all words from the current task
...
1. DONE: Possibility to get paradigms in text format using a console command
   * curl http://localhost:3000/paradigms/dump

2. DONE: in paradigms#dump, allow filtering
   * get only paradigms with status=ready or status=review.
   ...
   * what should be the default? dumping only ready paradigms? or dumping all paradigms -- dumping all, regardless of the status

3. in paradigms#dump, mark paradigms that have already been dumped
   * dumping procedure should modify the database. what is better for this purpose: get or post?

4. when showing individual words (with tags), how to treat comments, info on typo (see other fields of Word)? Should typos be filtered out somehow in dumping or leave it to postprocessing?

5. paradigms that have already been dumped should be visibly different from other paradigms. maybe to the extent that it should not be editable?

6. Now dumping paradigms does not require authentication. Implement one for it? consider Basic HTTP Authentication

== TODO for paradigms

1. find/show all paradigms with unfilled slots, having comments and specific status (Ready, Review)

2. if a paradigm has been dumped, show it somethow in #edit
...
6. assign task statuses, more particuraly, READY, and hide READY tasks by default
...
2. How to add (in bulk) new words/tasks?

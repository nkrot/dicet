...
5. INPROGRESS: Add task_id to Paradigm. this will help computing task statuses
   * DONE: add the field to the Paradigm model.
   * DONE: add logic for linking paradigms to tasks -- added ParadigmForm.save
...
6. INPROGRESS: assign task statuses, more particularly, 'new', 'inprogress', 'review', 'hascomment' and 'ready'. 
   * compute task statuses and save them in Task.status. this should happem on every paradigm save/delete
   * DONE: on the User's page, display user unfinished (inprogress, review and hascomment) tasks first and then not-started tasks.
   * additional page for ready tasks
   * paginate not started tasks (status new). Do not show unfinished tasks on the second and firther pages of new-tasks display.
   * in the table that displays tasks, in the status column, perhaps it would be nice to have counts of words that are done, have comments and are to review? again, highlight using different colors.

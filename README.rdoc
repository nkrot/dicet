...
4. [+] How to create more that one paradigm with the same noun (Band: Bänder/Bande)?
...
12. INPROGRESS: paradigms#edit edits only one (selected) paradigm.
   * when saved, where does it redirect? -- should probably remain on the same page (similarly as DELETE button). use ajax?
   * allow renaming?
   * allow undeleting? at least just after it was deleted on paradigms#edit
   * BUG: display tags that are not initially in paradigm of the given type (ex: an NN pdg was exxtended with a JJ tag, the JJ tag should also be displayed in #edit)
...
18. INPROGRESS:in paradigms#edit, allow changing word text and word tag
  * DONE: changing word text
  * changing the word tag -- needs testing

19. DONE: when deleting the whole paradigm, disable the DELETE abd SAVE buttons after the first use because they should not work when clicked again. Alternatively, allow undeleting.

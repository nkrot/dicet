...
7. INPROGRESS: Implement navigation through the words of the current task in a side pane
  * DONE: show the current task content in #new
  * DONE: highlight the current word
  * DONE: update changed word dynamically
  * DONE: use bullets of different colors to show whether the paradigm has comments or has review status
  * add the side pane to the #edit view
  * keep current_task in a session rather than carry it in links?
...
3. NEXT: (important!) allow marking a word as a typo and allow adding the correct variant
